...
FEM::FEM(const Mesh1D& FEmesh, int order)
  : mesh(FEmesh),
  meshSize(mesh.size),
...
FEM::FEM(const Mesh1D& FEmesh, int order, realFunction initialCondition)
  : FEM(FEmesh, order)
{
...
std::vector<int>& FEM::operator[](const int elementIndex)
{
  // Debug
...
const std::vector<int>& FEM::operator[](const int elementIndex) const
{
  // Debug
  ASSERT(elementIndex >= 0, "Element index must be non-negative");
  ASSERT(elementIndex < meshSize, "Element index must be less than the number of elements");
...
FENode& FEM::operator()(const int elementIndex, const int nodeIndex) const
{
  // Debug
  ASSERT(elementIndex >= 0, "Element index must be non-negative");
  ASSERT(elementIndex < meshSize, "Element index must be less than the number of elements");
...
real FEM::evaluate(const real x) const
{
  return evaluate(x, 0);
}

real FEM::integrateElement(const int elementIndex, const int n_gq) const
{
  // Debug
  ASSERT(elementIndex >= 0, "Element index must be non-negative");
  ASSERT(elementIndex < meshSize, "Element index must be less than the number of elements");
...
real FEM::integrate(const int n_gq) const
{
  real sum = 0;
...
real FEM::evaluate(const real x, const int derivativeOrder) const
{
...
real FEM::evaluate(const real x, const int elementIndex, const int derivativeOrder) const
{
  const int& K = elementIndex;
...
void FEM::plot(const int n, const int derivativeOrder) const
{
