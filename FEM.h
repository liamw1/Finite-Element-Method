...
public:
  ...
  FENode* FENodes; // Stores x-values of all finite element nodes
  ...
  /*
    ...
    polynomial order.
  */
  FEM(const Mesh1D& mesh, int order);
  ...
  */
  FEM(const Mesh1D& mesh, int order, realFunction initialCondition);

  /*
    Move constructor.
  */
  FEM(FEM&& other) noexcept;

  FEM(const FEM& other) = delete;

  FEM& operator=(const FEM& other) = delete;
  ...
  */
  FENode& operator()(const int elementIndex, const int nodeIndex) const;
